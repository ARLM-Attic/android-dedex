    ...
    partial class DockPanel
    {
        private static class Persistor
        {
            ...
            public static void SaveAsXml(DockPanel dockPanel, Stream stream, Encoding encoding, bool upstream)
            {
                ...
                // will be able to be backwards compatible or at least recognize out of date versions
                xmlOut.WriteStartElement("DockPanel");
                xmlOut.WriteAttributeString("FormatVersion", ConfigFileVersion);
                if (dockPanel.ParentForm != null)
                {
                    xmlOut.WriteAttributeString("ParentWState", dockPanel.ParentForm.WindowState.ToString());
                    if (dockPanel.ParentForm.WindowState == FormWindowState.Minimized)
                    {
                        dockPanel.ParentForm.Hide();
                        dockPanel.ParentForm.WindowState = FormWindowState.Normal;
                    }

                    xmlOut.WriteAttributeString("ParentLeft", dockPanel.ParentForm.Left.ToString(CultureInfo.InvariantCulture));
                    xmlOut.WriteAttributeString("ParentTop", dockPanel.ParentForm.Top.ToString(CultureInfo.InvariantCulture));
                    xmlOut.WriteAttributeString("ParentWidth", dockPanel.ParentForm.Width.ToString(CultureInfo.InvariantCulture));
                    xmlOut.WriteAttributeString("ParentHeight", dockPanel.ParentForm.Height.ToString(CultureInfo.InvariantCulture));
                }
            ...
            public static void LoadFromXml(DockPanel dockPanel, Stream stream, DeserializeDockContent deserializeContent, bool closeStream)
            {
                ...
                FormWindowState parentWindowState;

                int parentTop = Convert.ToInt32(xmlIn.GetAttribute("ParentTop"), CultureInfo.InvariantCulture);
                int parentLeft = Convert.ToInt32(xmlIn.GetAttribute("ParentLeft"), CultureInfo.InvariantCulture);
                int parentWidth = Convert.ToInt32(xmlIn.GetAttribute("ParentWidth"), CultureInfo.InvariantCulture);
                int parentHeight = Convert.ToInt32(xmlIn.GetAttribute("ParentHeight"), CultureInfo.InvariantCulture);
                try
                {
                    parentWindowState =
                        (FormWindowState)Enum.Parse(typeof(FormWindowState), xmlIn.GetAttribute("ParentWState"));

                }
                catch (Exception e)
                {
                    parentWindowState = FormWindowState.Normal;
                }

                if (parentWindowState == FormWindowState.Minimized)
                {
                ...
                {
                    rectangle = Rectangle.Union(rectangle, screen.WorkingArea);
                    //rectangle..Inflate(-screen.WorkingArea.Width, -screen.WorkingArea.Height);
                }

                if (parentLeft > rectangle.Width)
                {
                    parentLeft = rectangle.Left;
                }

                if (parentTop > rectangle.Height)
                {
                    parentTop = rectangle.Top;
                }
                if (parentLeft < 0)
                {
                    parentLeft = 0;
                ...
                if (dockPanel.ParentForm != null)
                {
                    dockPanel.ParentForm.Left = parentLeft;
                    dockPanel.ParentForm.Top = parentTop;
                    if (parentWidth > 0)
                    {
                        dockPanel.ParentForm.Width = parentWidth;
                    }
                    if (parentHeight > 0)
                    {
                        dockPanel.ParentForm.Height = parentHeight;
                    }
                    dockPanel.ParentForm.WindowState = parentWindowState;
                }
